...
function Skeleton() {
  return (
    <div className="flex flex-col gap-3">
      {[1, 2, 3, 4, 5].map(i => (
        <div key={i} className="skeleton rounded-2xl" style={{ height: '64px', opacity: 1 - i * 0.15 }} />
      ))}
    </div>
  )
}
...
function FAQItem({ faq, index, isOpen, onToggle }) {
  return (
    <motion.div className="rounded-2xl overflow-hidden cursor-pointer"
      style={{
      ...
      onClick={onToggle} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
      transition={{ duration: 0.5, delay: index * 0.08, ease: [0.25, 0.46, 0.45, 0.94] }}
    >
      {/* Question row */}
      <div className="flex items-center justify-between px-6 py-5 gap-4">
        <h3 className="font-syne text-base md:text-lg font-bold leading-snug flex-1" style={{ color: isOpen ? '#212023' : '#FFFFFF' }}>
          {faq.question}
        </h3>
        <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-300"
          ...
          { isOpen ? <FiMinus size={14} color="#212023" /> : <FiPlus  size={14} color="#AABBC5" /> }
        </div>
      </div>
      ...
      <AnimatePresence initial={false}>
        { isOpen && (
          ...
          >
            <div className="px-6 pb-6" style={{ borderTop: '1px solid rgba(33,32,35,0.12)' }}>
              <p className="text-sm leading-relaxed pt-4" style={{ color: 'rgba(33,32,35,0.78)' }} >
                {faq.answer}
              </p>
            </div>
          </motion.div>
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-24 px-4 md:px-10" style={{ background: '#262626' }}>
